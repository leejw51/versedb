...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let mut db = SqliteDatabase::open("time_records.db").await?;
    ...
    // Demonstrate select_range by selecting records 3 to 7
    println!("\nSelecting records in range (3 to 7):");
    let start = "record_3".as_bytes();
    let end = "record_8".as_bytes(); // Note: end is exclusive, so this will include up to record_7
    let range_records = db.select_range(start, end).await?;
    for (key, value) in range_records {
        println!(
            "Key: {}, Value: {}",
